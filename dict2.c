...
struct athlete_t{
  ...
  struct athlete_t *left, *right, *parent, *middle;
};
...
struct athlete_t *insert(struct athlete_t *parent, struct athlete_t *new){
  ...
  /*new node's key is lower than the parent,
  ...
  /*new node's key equals to parent's key,
  go to the middle of the parent to build a linked list*/
  else{
    new->parent = parent;
    parent->middle = insert(parent->middle,new);
  }
  return parent;
}


/*function to read the csv file and make a bst fom the input data*/
...
int search(struct athlete_t *head, char key[MAX_STRING+1],
  ...
  if(head){
    ...
    if(str_comparsion(head->key, key)==0){
      /*Stop the key comparison. Traverse the linked list
      and write the data in each node to the outputfile*/
    ...
    /*key has lower order than the current node's key*/
      ...
      /*The node does not have a left child. No information found.
      End of search*/
      else{
    ...
    /*key has higher order than the current node's key*/
      ...
      /*The node has a right child, continue search in the right tree*/
      if(head->right){
        count += search(head->right, key, outputfile, found);
      }
      /*The node does not have a right child.
       No infprmation has been found. Write in the file.
      End of search.*/
      else{
        fprintf(outputfile,"%s --> NOTFOUND\n", key);
      }
